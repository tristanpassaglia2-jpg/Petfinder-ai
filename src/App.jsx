...
function PetForm({title,sub,type,onSubmit,onClose,analyzePhoto,analyzingPhoto,analysisResult,setAnalysisResult}){
  ...
  return (
    <div>
      ...
      {/* STEP 1: PHOTO */}
      {step===1&&(<div style={{display:"flex",flexDirection:"column",gap:12,animation:"fadeIn .3s"}}>
        <div onClick={()=>fileRef.current?.click()} style={{border:"2px dashed #E7E5E4",borderRadius:14,padding:preview?0:24,textAlign:"center",cursor:"pointer",background:preview?"none":"#FAFAF9",minHeight:130,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",position:"relative",overflow:"hidden"}}>
          {preview?<img src={preview} style={{width:"100%",height:180,objectFit:"cover",borderRadius:12}} alt=""/>:<><$.Cam s={28} c="#D6D3D1"/><div style={{marginTop:8,fontWeight:600,color:"#A8A29E",fontSize:13}}>Subir foto</div><div style={{fontSize:10,color:"#D6D3D1",marginTop:2}}>La IA analizará la imagen</div></>}
        ...
        <div style={{display:"flex",gap:7}}>{[{k:"dog",l:"Perro",i:<$.Dog s={16}/>},{k:"cat",l:"Gato",i:<$.Cat s={16}/>}].map(t=>(<button key={t.k} onClick={()=>up("type",t.k)} style={{flex:1,padding:12,borderRadius:12,border:d.type===t.k?"2px solid #E8590C":"2px solid #E7E5E4",background:d.type===t.k?"#E8590C08":"#fff",cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",gap:6,fontWeight:700,fontSize:13,color:d.type===t.k?"#E8590C":"#78716C",fontFamily:"inherit"}}>{t.i}{t.l}</button>))}</div>
